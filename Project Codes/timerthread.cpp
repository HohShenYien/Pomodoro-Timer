...
void TimerThread::requestWork(int duration, int short_rest, int long_rest, int session,
    ...
    if (notifOrNot) {
        system("notify-send 'Focus' 'Focus time start!' '-t' 5000");
    }
        ...
        if (((the_timer->cur_time == 0) & isRunning & notifOrNot)) {
            ...
            if (the_timer->status == 0) {
                ...
                if (the_timer->session == 1) {
                    // long break
                    system("notify-send 'Long Break' 'Time for a long break!' '-t' 5000");
                }else {
                        // short break
                        system("notify-send 'Short break' 'Time for a short break!' '-t' 5000");
                    }
            } else {
                // back to focus
                system("notify-send 'Focus' 'Focus time start!' '-t' 5000");
